import { plumbingData } from "../backend/plumbing.data.js";
...
function waitForElement(selector, cb) {
  ...
  setTimeout(() => {
    if (!document.querySelector(selector)) {
      console.warn("[plumbing] still waiting for", selector);
    }
  }, 5000);
}
...
function createCard(card) {
  ...
  li.className = "plumbing-slide splide__slide";

  const art = document.createElement("article");
  art.className = "plumbing-card";

  const logosWrap = document.createElement("div");
  logosWrap.className = "plumbing-container-card-img";
  ...
  card.brands.forEach((b) => {
    ...
    a.target = "_blank";
    a.rel = "noopener";
    a.className = "plumbing-link-logo";

    const img = document.createElement("img");
    img.src = b.img;
    img.alt = b.alt || b.name || "";
    img.className = b.class || "plumbing-logo";
    img.loading = "lazy";
  ...
  textWrap.className = "plumbing-container-carusel-text";

  const h3 = document.createElement("h3");
  h3.className = "plumbing-card-title";
...
function renderPlumbing(track, data = plumbingData) {
...
function fitLogosContainer(wrap) {
  const items = wrap.querySelectorAll(".plumbing-link-logo");
  ...
  if (!H || H < 40) {
    const card = wrap.closest(".plumbing-card");
    const title = card?.querySelector(".plumbing-container-carusel-text");
...
function fitAll() {
  document
    .querySelectorAll(".plumbing-container-card-img")
...
function watchImagesAndResize() {
  const imgs = document.querySelectorAll(".plumbing-link-logo img");
...
function setupTailAutoHeight(root, splide, section = "plumbing") {
  ...
  function measureHeights() {
    ...
    if (rows === 2 && cols === 1) {
      ...
      halfHeight = cardH;
      if (!track.style.height) track.style.height = baseHeight + "px";
    } else {
      baseHeight = cardH;
      halfHeight = cardH;
    }
...
function setTrackHeight(root) {
  const track = root.querySelector(".splide__track");
  if (!track) return;
  if (window.innerWidth >= 1440) {
    track.style.height = "607px";
  }
  if (window.innerWidth >= 1920) {
    track.style.height = "622px";
  } else {
    track.style.height = "";
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  waitForElement("#plumbing-track", (track) => {
    renderPlumbing(track);
    ...
    const root = document.getElementById("plumbing-splide");
    if (!root) return;

    const splide = new Splide("#plumbing-splide", {
      type: "loop",
      rewind: false,
      perMove: 1,
      direction: "ltr",
      ...
      breakpoints: {
        ...
        1919: {
          ...
          fixedWidth: "100%",
          gap: "15px",
          grid: { rows: 2, cols: 2, gap: { row: "15px", col: "15px" } },
        },
        1439: {
          gap: "15px",
    ...
    setTrackHeight(root);
    window.addEventListener("resize", () => setTrackHeight(root));

    setTimeout(fitAll, 0);

    const prevBtn = document.getElementById("plumbing-button-left");
    const nextBtn = document.getElementById("plumbing-button-right");
