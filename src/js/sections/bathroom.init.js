...
import { bathroomData } from "@/js/backend/bathroom.data.js";
...
function waitForElement(selector, cb) {
  ...
  setTimeout(() => {
    if (!document.querySelector(selector)) {
      console.warn("[bathroom] still waiting for", selector);
    }
  }, 5000);
}
...
function createCard(card) {
  ...
  li.className = "bathroom-slide splide__slide";

  const art = document.createElement("article");
  art.className = "bathroom-card";

  const logosWrap = document.createElement("div");
  logosWrap.className = "bathroom-container-card-img";
  ...
  card.brands.forEach((b) => {
    ...
    a.target = "_blank";
    a.rel = "noopener";
    a.className = "bathroom-link-logo";

    const img = document.createElement("img");
    img.src = b.img;
    img.alt = b.alt || b.name || "";
    img.className = b.class || "bathroom-logo";
    img.loading = "lazy";
  ...
  textWrap.className = "bathroom-container-carusel-text";

  const h3 = document.createElement("h3");
  h3.className = "bathroom-card-title";
...
function renderBathroom(track, data = bathroomData) {
  console.log("[bathroom] rendering", data.length, "cards");
...
function fitLogosContainer(wrap) {
  const items = wrap.querySelectorAll(".bathroom-link-logo");
  ...
  if (!H || H < 40) {
    const card = wrap.closest(".bathroom-card");
    const title = card?.querySelector(".bathroom-container-carusel-text");
...
function centerTail(wrap, cols, n) {
  wrap.querySelectorAll(".bathroom-link-logo").forEach((el) => {
    el.style.gridColumn = "";
  ...
  if (lastRowCount === 1) {
    ...
    const last = wrap.querySelector(".bathroom-link-logo:last-child");
    if (last) {
  ...
  if (lastRowCount === 2) {
    ...
    const secondLast = wrap.querySelector(
      ".bathroom-link-logo:nth-last-child(2)"
    );
    const last = wrap.querySelector(".bathroom-link-logo:last-child");
...
function fitAll() {
  document
    .querySelectorAll(".bathroom-container-card-img")
...
function watchImagesAndResize() {
  const imgs = document.querySelectorAll(".bathroom-link-logo img");
...
document.addEventListener("DOMContentLoaded", () => {
  waitForElement("#bathroom-track", (track) => {
    console.log("[bathroom] track found → rendering");
    renderBathroom(track);
    ...
    const root = document.getElementById("bathroom-splide");
    if (!root) return;

    const splide = new Splide("#bathroom-splide", {
      type: "loop",
      perMove: 1,
      direction: "ltr",
      arrows: false,
      ...
      breakpoints: {
        9999: {
          gap: "10px",
          grid: { rows: 1, cols: 2, gap: { row: "10px", col: "10px" } },
        },
        1919: {
          gap: "15px",
          grid: { rows: 1, cols: 2, gap: { row: "15px", col: "15px" } },
    ...
    const prevBtn = document.getElementById("bathroom-button-left");
    const nextBtn = document.getElementById("bathroom-button-right");
